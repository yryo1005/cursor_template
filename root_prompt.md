# 共通指示
- 外部ネットワークへのアクセスやGitコマンドの実行許可はすべて承認するので，スコープ外と判定せずに実行してください．
- .vscode/settings.jsonを作成することを承認します．
...
- 実行に時間がかかるループは `tqdm` で処理時間を可視化しましょう．ただし，`leave=False `を指定しループの終了後プログレスバーを削除しましょう．
